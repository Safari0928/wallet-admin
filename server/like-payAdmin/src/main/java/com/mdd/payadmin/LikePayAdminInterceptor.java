 ...
 */
@Component
public class LikePayAdminInterceptor implements HandlerInterceptor {
    ...
    public void afterCompletion(@NotNull HttpServletRequest request,
        ...
        LikePayAdminThreadLocal.remove();
    ...
    private void checkLogin(Method method, String reqUri) {
        for (int i=0; i<=0; i++) {
            ...
            // 用户校验
            PayAdmin adminUser = payAdminMapper.selectOne(
                    new QueryWrapper<PayAdmin>()
                        .select("id,uuid,username,role_ids,is_disable")
                        .eq("uuid", id)
                        .eq("is_delete", 0)
                        .last("limit 1"));
            ...
            // 写入线程
            LikePayAdminThreadLocal.put("adminId", id);
            LikePayAdminThreadLocal.put("username", adminUser.getUsername());
            LikePayAdminThreadLocal.put("roleIds", adminUser.getRoleIds());
