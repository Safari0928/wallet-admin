 ...
 */
@Component
public class LikeMobileInterceptor implements HandlerInterceptor {
    ...
    private void checkLogin(Method method) {
        for (int i=0; i<=0; i++) {
            ...
            if (StringUtils.isNotNull(method) && method.isAnnotationPresent(NotLogin.class)) {
                try {
                    ...
                    if (StringUtils.isNotNull(id)) {
                        ...
                        String uid = StringUtils.isNull(user) ? "0" : userId;
                        LikeMobileThreadLocal.put("userId", uid);
                    }
            ...
            String  userId =id.toString();
            PayUser user = payUserMapper.selectOne(new QueryWrapper<PayUser>()
                    .select("uuid,phone_number,email,status,pay_id")
                    .eq("uuid", userId)
                    .eq("status", 1)
                    .last("limit 1"));
            ...
            if (StringUtils.isNull(user)) {
                Integer errCode = ErrorEnum.TOKEN_INVALID.getCode(); //TODO Create err code and message
                String errMsg = ErrorEnum.TOKEN_INVALID.getMsg();
                throw new LoginException(errCode, errMsg);
            }
            ...
            if (user.getStatus().equals(-1)) {
                Integer errCode = ErrorEnum.LOGIN_DISABLE_ERROR.getCode(); //TODO Create err code and message
                String errMsg = ErrorEnum.LOGIN_DISABLE_ERROR.getMsg();
                throw new LoginException(errCode, errMsg);
            }

            // 写入线程
            LikeMobileThreadLocal.put("userId", user.getUuid());
            LikeMobileThreadLocal.put("email", user.getEmail());
            LikeMobileThreadLocal.put("phoneNumber", user.getPhoneNumber());
            LikeMobileThreadLocal.put("status", user.getStatus());
            LikeMobileThreadLocal.put("payId",user.getPayId());
        }
    }
